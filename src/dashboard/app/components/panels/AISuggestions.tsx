...
const sections = [
  { key: "achievements", title: "Achievements", color: "#059669" },
  { key: "improvements", title: "Improvements", color: "#3b82f6" },
  { key: "next_tasks", title: "Next Tasks", color: "#d97706" },
  { key: "risks", title: "Risks & Tech Debt", color: "#dc2626" },
] as const;

export function AISuggestions({ data }: { data: OwlData }) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-sm max-w-sm" style={{ color: "var(--text-muted)" }}>
          Run <code style={{ color: "var(--text-secondary)" }}>openowl cron run project-suggestions</code> to generate now.
        </p>
      </div>
    );
  }
        ...
        {sections.map(({ key, title, color }) => {
          ...
          return (
            <div key={key} className="rounded-xl p-5" style={{ background: `${color}08`, border: `1px solid ${color}33` }}>
              <h3 className="font-medium mb-3" style={{ color }}>{title}</h3>
              {items.length === 0 ? (
