...
export function TokenUsage({ data }: { data: OwlData }) {
  ...
  const withoutOwl = totalTracked + savings;
  const withBareCli = withoutOwl;
  const savingsPercent = withoutOwl > 0 ? Math.round((savings / withoutOwl) * 100) : 0;

  const comparisonData = [
    { name: "Without OpenOwl", tokens: withBareCli, fill: "#fbbf24" },
    { name: "OpenOwl + OpenCode", tokens: totalTracked, fill: "#34d399" },
  ];
      ...
      <div className="rounded-xl p-5 mb-6" style={{ background: "var(--bg-surface)", border: "1px solid var(--border)" }}>
        ...
        <ResponsiveContainer width="100%" height={150}>
          <BarChart data={comparisonData} layout="vertical">
            ...
            <Tooltip contentStyle={{ background: "var(--chart-tooltip-bg)", border: "1px solid var(--chart-tooltip-border)", borderRadius: 8, color: "var(--text-primary)" }} formatter={((v: number) => [formatTokens(v) + " tokens", ""]) as never} />
