...
function buildTree(entries: OwlData["anatomy"]["entries"]): TreeNode {
    ...
    <div className="ml-3">
      {node.name !== "." && (
        <button onClick={() => setExpanded(!expanded)} className="flex items-center gap-1.5 py-1 text-sm transition-colors"
          ...
          <span className="w-4 text-center" style={{ color: "var(--text-faint)" }}>{expanded ? "v" : ">"}</span>
          <span>{node.name}/</span>
...
export function AnatomyBrowser({ data }: { data: OwlData }) {
  const { anatomy } = data;
      ...
      <div className="rounded-xl p-4" style={{ background: "var(--bg-surface)", border: "1px solid var(--border)" }}>
        {anatomy.entries.length === 0 ? (
          <div className="text-center py-12" style={{ color: "var(--text-muted)" }}>
            <p className="text-2xl mb-2">[dir]</p>
            <p>No anatomy data. Run <code style={{ color: "var(--text-secondary)" }}>openowl scan</code> to index your project.</p>
          </div>
        ) : (
