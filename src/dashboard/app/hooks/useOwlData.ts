...
import { OwlClient } from "../lib/owl-client.js";
...
interface BugLog { bugs: any[]; }
interface CronManifest { tasks: any[]; }
interface DesignQCReport { captured_at: string | null; captures: any[]; total_size_kb: number; estimated_tokens: number; }

interface Health { status: string; uptime_seconds: number; }
interface ProjectMeta { name: string; description: string; root: string; }
...
export function useOwlData(): OwlData {
  const [loading, setLoading] = useState(true);
  const [anatomy, setAnatomy] = useState<OwlData["anatomy"]>({ entries: [], metadata: { files: 0, hits: 0, misses: 0 } });
  ...
  const [identity, setIdentity] = useState({ name: "Owl", role: "AI development assistant" });
  const [project, setProject] = useState<ProjectMeta>({ name: "", description: "", root: "" });
  const [client, setClient] = useState<OwlClient | null>(null);
  ...
  const processFiles = useCallback((files: Record<string, string>) => {
    ...
    if (files["token-ledger.json"]) { try { setTokenLedger(JSON.parse(files["token-ledger.json"])); } catch {} }
    if (files["cron-state.json"]) { try { setCronState(JSON.parse(files["cron-state.json"])); } catch {} }
    if (files["cron-manifest.json"]) { try { setCronManifest(JSON.parse(files["cron-manifest.json"])); } catch {} }
    if (files["buglog.json"]) { try { setBuglog(JSON.parse(files["buglog.json"])); } catch {} }
    if (files["suggestions.json"]) { try { setSuggestions(JSON.parse(files["suggestions.json"])); } catch {} }
    if (files["designqc-report.json"]) { try { setDesignqcReport(JSON.parse(files["designqc-report.json"])); } catch {} }
    if (files["identity.md"]) {
      const nameMatch = files["identity.md"].match(/\*\*Name:\*\*\s*(.+)/);
      const roleMatch = files["identity.md"].match(/\*\*Role:\*\*\s*(.+)/);
      if (nameMatch || roleMatch) {
        setIdentity({
          name: nameMatch?.[1]?.trim() || "Owl",
  ...
  useEffect(() => {
    fetch("/api/files").then(r => r.json()).then(files => { processFiles(files); setLoading(false); }).catch(() => setLoading(false));
    fetch("/api/health").then(r => r.json()).then(h => setHealth(h)).catch(() => {});
    fetch("/api/project").then(r => r.json()).then(p => setProject(p)).catch(() => {});

    const wsClient = new OwlClient();
    wsClient.connect();
    setClient(wsClient);

    wsClient.onMessage((msg) => {
      if (msg.type === "file_changed") processFiles({ [msg.file]: msg.content });
      if (msg.type === "full_state" && msg.files) processFiles(msg.files);
      if (msg.type === "health") setHealth({ status: msg.status, uptime_seconds: msg.uptime });
    });
