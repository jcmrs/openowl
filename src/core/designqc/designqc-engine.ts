...
export class DesignQCEngine {
  ...
  constructor(owlDir: string, projectRoot: string, options: DesignQCOptions) {
    this.owlDir = owlDir;
  ...
  async capture(): Promise<CaptureResult> {
    ...
    try {
      ...
      const captureDir = path.join(this.owlDir, "designqc-captures");
      ...
      try {
        ...
        console.error("puppeteer-core is required for designqc. Install with: npm install puppeteer-core");
        return { screenshots: [], captureDir: "", totalSizeKB: 0, estimatedTokens: 0 };
      }
      ...
      console.log("");
      console.log("  Ask the AI: \"Read the screenshots in .owl/designqc-captures/ and evaluate the design\"");

      const now = new Date();
      const timeStr = `${String(now.getHours()).padStart(2, "0")}:${String(now.getMinutes()).padStart(2, "0")}`;
      appendText(
        path.join(this.owlDir, "memory.md"),
